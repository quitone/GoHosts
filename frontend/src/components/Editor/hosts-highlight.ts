...
export function toggleCommentBySelection(
  ...
  const endLineIndex = start === end ? startLineIndex : getLineIndexAtOffset(lines, Math.max(start, end - 1))

  return toggleCommentLines(normalizedCode, selectionStart, selectionEnd, startLineIndex, endLineIndex, moveToNextLine)
}

/** Toggle comment on a single line identified by its zero-based index (used for gutter clicks). */
export function toggleCommentByLine(code: string, lineIndex: number, selectionStart: number, selectionEnd: number): CommentToggleResult {
  const normalizedCode = normalizeLineEndings(code)
  const lines = getLines(normalizedCode)
  ...
  return toggleCommentLines(normalizedCode, selectionStart, selectionEnd, lineIndex, lineIndex, false)
}
